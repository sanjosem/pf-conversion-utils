...
for required_field in ['directory','fnc_filename','probe_list']:
...
if uinfo['probe_list'] is not None:
    for probe_name in uinfo['probe_list'].keys():
        probe_coords = uinfo['probe_list'][probe_name]
        fncConv.extract_probe(probe_name,probe_coords)
    fncConv.export_temporal_data(uinfo['casename'],uinfo['output_directory'])
